...
At this point, you could jump ahead to the _<<run>>_ section and run the application.
...
The `configure(HttpSecurity)` method defines with URL paths should be secured and which should not. Specifically, the "/" and "/home" paths are configured to not require any authentication. All other paths must be authenticated. 
